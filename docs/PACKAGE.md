...
| `Review → UserSchedule` (Application 경유 read) | ✅ | `ReviewCommandService` / `ReviewQueryService`가 `UserScheduleQueryService.currentMode(userId)`를 호출해 사용자 현재 모드(`LearningMode`)를 조회한다. UserSchedule 엔티티는 BC 경계 밖으로 노출하지 않고 `LearningMode` enum만 반환. **Archive 판정은 Card BC 밖(M5 DailyLearningBatch)으로 이관됨** — Story-CARD-E2, 2026-07-03, `OnFieldBudget` VO 폐기 |
...
| `Deck → LearningFacade` (이벤트 import) | ✅ | `LearningAxisCreatedEvent` / `LearningMaterialDeletedEvent` 수신 — 동기 도메인 이벤트 협력 ([ADR007](adr/ADR007.md)). **fix-axis-deck-full-integration 0.0.2v/ADR021 이후 `LearningAxisCreatedEvent` 수신은 Deck 생성의 유일 진입점** — 사용자 명시 진입점은 폐기됨 |
| `LearningFacade → Deck` (Application 경유 read + 소프트 삭제 조율) | ✅ | (1) `LearningFacadeQueryService`가 `DeckQueryService.findByAxisIds(...)` 호출 — facade 응답의 축별 linkedDecks 매핑(Story-005-2). (2) fix-axis-deck-full-integration 0.0.2v/ADR021: `LearningFacadeCommandService.removeAxis`가 `DeckCommandService.softDeleteByAxisId(axisId)` 호출 — Axis softDelete 후 소속 Deck 연쇄 소프트 삭제 조율 (동일 `@Transactional`, `facade save → Deck 연쇄` 순서). **Repository 직접 호출 금지** — Application Service 경유만 |
...
| v0.3 | 2026-07-01 | fix-axis-deck-full-integration (0.0.2v) 반영. `Deck → LearningFacade` 이벤트 수신을 `LearningAxisCreatedEvent`로 재명명하고 **Deck 생성의 유일 진입점**임을 명시. `LearningFacade → Deck`에 소프트 삭제 조율 경로(`softDeleteByAxisId` Application 호출) 추가 (ADR021) |
