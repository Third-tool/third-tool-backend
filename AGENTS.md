...
> Codex의 단일 진입 포인터. 작업 흐름·구조·컨벤션의 본질은 본 파일에서 시작해 해당 룰·문서로 연결된다.
...
| 무엇을 알고 싶을 때 | 어디를 보나 |
| --- | --- |
| **작업 흐름 / Story 실행 절차** | [`.Codex/rules/workflow.md`](.Codex/rules/workflow.md) |
| **도메인 의도·용어·불변식** | [`docs/DOMAIN.md`](docs/DOMAIN.md) |
| **패키지·BC·레이어 구조** | [`docs/PACKAGE.md`](docs/PACKAGE.md) |
| **아키텍처 결정 기록 (ADR)** | [`docs/adr/`](docs/adr/) |
| **코드 작성 컨벤션** (도메인·API·DB·테스트) | [`.Codex/rules/conventions.md`](.Codex/rules/conventions.md) |
| **Story Reviewer 세션** (5관점 병렬 검토) | [`.Codex/rules/review.md`](.Codex/rules/review.md) |
| **브랜치·커밋·push·PR 규칙** | [`.Codex/rules/pr-commit.md`](.Codex/rules/pr-commit.md) |
| **ADR 작성 트리거** | [`.Codex/rules/adr.md`](.Codex/rules/adr.md) |
| **현재 진행 중인 Epic / Story** | `workflow/epics/Epic.md` · `workflow/stories/Story.md` |

본 AGENTS.md의 다른 섹션은 위 룰·문서와 모순될 경우 **룰·문서가 우선**한다 — AGENTS.md는 빠른 개요이지 단일 진실 소스가 아니다.
...
별도 명세 문서를 만들지 말 것. 위 영역의 변경은 docs 갱신을 묻지 않는다 ([`.Codex/rules/workflow.md`](.Codex/rules/workflow.md) Step 5 표 참조).
...
### 브랜치 명명
`<type>/<NNN>-<short-kebab-case>` (Epic 1개 = 브랜치 1개, 상세는 [`pr-commit.md`](.Codex/rules/pr-commit.md))
...
## 머지 정책
- 머지는 사람이 직접. Codex는 PR 생성·머지 명령을 실행하지 않는다.

---

## 작업 흐름 (요약)

Story 명령 수신 시 [`workflow.md`](.Codex/rules/workflow.md) Step 1~5 강제:
...
- `update-docs/` — Codex 기록 출력 (architecture·dict·table-spec·test·adr). 폐기. ADR은 `docs/adr/` tracked로 정착.
- `docs/architecture/`, `docs/database/` — 흡수/폐기 후 `docs/PACKAGE.md` 단일 파일로 정착.
- `.Codex/rules/private-docs.md`, `update-docs.md`, `update-docs/*.md`, `{domain,api,db,test}-conventions.md` — 모두 폐기. `conventions.md` + `review.md` + `adr.md`로 압축.
